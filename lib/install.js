...
var userAgent = utilnode.format('lego-client (%s, %s, %s %s)',
...
function* installPackage(id, args, saveDeps) {
  ...
  if (args.force || !(cacheIsExists && md5file(fileInCache) === pinfo.md5)) {
    ...
    var downArgs = {};
    downArgs.headers = {};
    downArgs.headers['user-agent'] = userAgent;
    yield download(fileInRemote, fileInCache, downArgs);
  }
...
function* dealDownloadIgnore(dest){
  // if -all or -a in argv, then return!
  var paramArr = process.argv.slice(2),
      keepAll = false;

  if(paramArr.indexOf('-all') > -1 || paramArr.indexOf('--all') > -1 || paramArr.indexOf('-a') > -1){
    return;
  }

  // if exist .dlignore
  var downloadignorePath = path.join(dest, '.dlignore');

  fs.stat(downloadignorePath, function(err, stats) {
    if (err) {
      return;
    }

    var content = fs.readFileSync(downloadignorePath, 'utf8');
    // console.log(content);

    var deleteFileOrFolder = function(path) {
    ...
    fileArr.forEach(function(item) {
      var curPath = path.join(dest, item);
